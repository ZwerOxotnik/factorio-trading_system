...
local function remove_command(command_name)
	if commands.remove_command("TSZO_" .. command_name) then
		return true
	else
...
local function disable_setting(error_message, player_index, orig_command_name)
	...
	if orig_command_name then
			local setting_name = "TSZO_" .. orig_command_name
...
local function add_custom_command(orig_command_name, command_settings, original_func)
	...
	if commands.commands[command_name] then
		command_name = "TSZO_" .. command_name
		if commands.commands[command_name] then
			return false
		end
	end

	local command_description = command_settings.description or {"trading_system-commands." .. command_settings.name}
...
function M:handle_custom_commands(module)
	...
	for command_name, func in pairs(module.commands) do
		...
		if SWITCHABLE_COMMANDS[command_name] then
			setting = settings.global["TSZO_" .. command_name]
		end
...
local function on_runtime_mod_setting_changed(event)
	...
	if find(event.setting, '^TSZO_') ~= 1 then return end
...
function M:create_settings()
	...
	for key, command in pairs(SWITCHABLE_COMMANDS) do
		...
		local description = command.description or {"trading_system-commands." .. command_name}
